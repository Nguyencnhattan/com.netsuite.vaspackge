 ...
 * @NScriptType Restlet
 */
define(["require", "exports", "N/log", "N/query", "../Include/constants.helper"], function (require, exports, log, query, constants_helper_1) {
    "use strict";
    ...
    function get(context) {
        ...
        switch (context.vasName) {
            case 'VAS-01: Trial Balance':
                const vfromDate2 = (0, constants_helper_1.getDDMMYYYY)(context.P_FROM_DATE);
                const vtoDate2 = (0, constants_helper_1.getDDMMYYYY)(context.P_TO_DATE);
      ...
      `;
               ...
               FROM ( SELECT ROWNUM AS ROWNUMBER, * FROM (` +
                ...
                if (records.length > 0) {
                    ...
                    const tempResult1 = (0, constants_helper_1.createNestedStructure)(filteredData);
                    const tempResult2 = (0, constants_helper_1.flattenTree)(tempResult1, filteredData);
                    outputTrialBalance = (0, constants_helper_1.calculateSummary)(tempResult2);
                    totalSummary = {
                        totalObDebit: 0,
                        totalObCrebit: 0,
                        totalDebit: 0,
                    ...
                    log.debug('outputTrialBalance', outputTrialBalance[231]);
                    let result = outputTrialBalance.map((item) => ({
                        'account': item.acctnumber,
                        'account_name': item.accountname,
                        'no_dau_ky': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.obdebit)).toFixed(2)) || 0,
                        'co_dau_ky': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.obcredit)).toFixed(2)) || 0,
                        'phat_sinh_no': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.debit)).toFixed(2)) || 0,
                        'phat_sinh_co': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.credit)).toFixed(2)) || 0,
                        'du_no_cuoi_ky': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.ebdebit)).toFixed(2)) || 0,
                        'du_co_cuoi_ky': (0, constants_helper_1.formatNumberWithCommas)(Number(Number(item.ebcredit)).toFixed(2)) || 0,
                        'bold': item.bold
                    }));
                    
                    var activeColumnIndexes = [];
                    log.debug('result', result[231]);
